...
real, allocatable:: y(:), obs(:), previous_x(:), x(:), c(:), delta(:), atc(:), g(:), a(:,:), ata(:,:), L(:,:)
integer:: n, t, i, j
...
allocate (y(n),obs(n),previous_x(t),x(t),c(n),delta(t),a(n,t),atc(t),ata(t,t),g(n),L(t,t))
...
open(unit= 4, status= 'old', action= 'read', file= 'initial_x2.dat')
read(4,*) previous_x(:)
close(4)

! lambda
open(unit= 5, status= 'old', action= 'read', file= 'lambda2.dat')
read(5,*) lambda
close(5)
...
deallocate (y,obs,previous_x,x,c,delta,a,ata,atc,l)
