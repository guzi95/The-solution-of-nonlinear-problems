!Este programa calcula o problema inverso pelo metodo de Gauss-Newton.
!Em posse dos dados observado em campo (aqui sao componentes gravitacionais) deseja-se estimar a
!profundidade e o raio da esfera em subsuperficie atraves de um processo iterativo.
...
open (unit= 4, file='initial_x3.dat', status='old', action='read')
read(4,*) x(:)
close(4)

! "Passo" em direção ao mínimo.
...
! Resíduos.
open(unit= 8, file='c_resíduo.dat', status='old', action='write')
